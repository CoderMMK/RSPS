...
public class PlayerAssistant {
	...
	public static void addClueReward(Client c, int clueLevel) {
		...
		if (clueLevel == 0) {
			...
			case 2:
				displayReward(c, mediemLevelReward[Misc.random(13)], 1,
						mediemLevelReward[Misc.random(13)], 1, -1, 1);
				break;
			}
		} else if (clueLevel == 2) {
			switch (chanceReward) {
			...
			case 2:
				displayReward(c, highLevelReward[Misc.random(75)], 1,
						highLevelReward[Misc.random(60)], 1, -1, 1);
				break;
			}
		}
	}
	...
	public void normYell(String Message) {
		for (int j = 0; j < PlayerHandler.players.length; j++) {
			if (PlayerHandler.players[j] != null) {
				...
				c2.sendClan(Misc.optimizeText(c.playerName), Message, "Ardi",
						c.playerRights);
			}
		}
	}
	...
	public void loadQuests() {
		...
		sendFrame126("", 29169);
		sendFrame126("@or1@Quest tab still under development", 29171);
		sendFrame126("@or1@More Information will be added", 29172);
		sendFrame126("@or1@to the tab soon", 29173);
		sendFrame126("", 29174);
	}
				...
				if (c.killerId != c.playerId)
					if (c.inWild()) {
						if (!PlayerKilling.hostOnList(o, c.connectedFrom)) {
							...
							o.KC++;
							o.pkp += 4;
							o.getItems().addItem(995, 500000);
							o.sendMessage("You have received @red@4 @bla@Pk Points after defeating @red@"
									+ Misc.optimizeText(c.playerName) + "!");
							o.sendMessage("You have recieved a extra of: @red@500,000@bla@ coins.");
							c.getPA().loadQuests();
							o.getPA().loadQuests();
						} else {
	...
	public void startTeleport(int x, int y, int height, String teleportType) {
		...
		if (c.inWild() && c.underAttackBy > 0
			...
			c.sendMessage("You need 2 law runes to teleport during a combat in the wilderness!");
			return;
		}
		if (c.inWild() && c.underAttackBy > 0
				&& c.getItems().playerHasItem(563, 2)) {
	...
	public void addStarter() {
		c.newPlayerAct = 1;
		c.getDH().sendOption2("Teleport me Home, I wan't to start playing!",
				"I'd rather stay here and do nothing.");
		// c.isNewPlayer = true;
		if (!Connection
