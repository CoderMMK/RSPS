...
public class Herblore {
	...
	private int[][] information2 = { { 249, 121, 2428, 1, 25 },
			...
			{ 2998, 6687, 6685, 81, 180 }, { 17520, 15332, 15332, 81, 1000 },
			{ 5972, 15300, 15300, 81, 500 } };
	...
	private void idHerb(int slot) {
		if (c.getItems().playerHasItem(information[slot][0])) {
			if (c.playerLevel[c.playerHerblore] >= information[slot][2]) {
				...
				c.getPA().addSkillXP(
						information[slot][3] * Config.HERBLORE_EXPERIENCE,
						c.playerHerblore);
				c.sendMessage("You identify the herb as a "
						+ c.getItems().getItemName(information[slot][1]) + ".");
	...
	private void makePot(int herbId) {
		if (c.getItems().playerHasItem(227)
			...
			if (c.playerLevel[c.playerHerblore] >= information2[slot][3]) {
				 ...
				 * c.getItems().getItemName(information2[slot][1]) + "."); }
				 * else {
				 */
				c.getItems().addItem(information2[slot][2], 1);
				c.sendMessage("You make a "
						+ c.getItems().getItemName(information2[slot][2]) + ".");
				// }
				c.getPA().addSkillXP(
						information2[slot][4] * Config.HERBLORE_EXPERIENCE,
						c.playerHerblore);
